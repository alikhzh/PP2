import pygame
import math

def main():
    pygame.init()
    screen = pygame.display.set_mode((800, 600))
    pygame.display.set_caption("Paint Application")

    # Цвета
    colors = {
        'red': (255, 0, 0),
        'green': (0, 255, 0),
        'blue': (0, 0, 255),
        'black': (0, 0, 0),
        'white': (255, 255, 255)
    }

    current_color = colors['black']
    mode = 'brush'   # текущий инструмент
    drawing = False  # зажата ли мышь

    # Холст
    canvas = pygame.Surface((800, 600))
    canvas.fill(colors['white'])

    # UI
    ui_rect = pygame.Rect(0, 0, 800, 50)

    # Кнопки цветов
    btn_red = pygame.Rect(10, 10, 30, 30)
    btn_green = pygame.Rect(50, 10, 30, 30)
    btn_blue = pygame.Rect(90, 10, 30, 30)
    btn_black = pygame.Rect(130, 10, 30, 30)

    # Кнопки инструментов
    btn_brush = pygame.Rect(200, 10, 60, 30)
    btn_rect = pygame.Rect(270, 10, 60, 30)
    btn_circ = pygame.Rect(340, 10, 60, 30)
    btn_erase = pygame.Rect(410, 10, 60, 30)

    # НОВЫЕ кнопки
    btn_square = pygame.Rect(480, 10, 60, 30)
    btn_rtri = pygame.Rect(550, 10, 60, 30)
    btn_etri = pygame.Rect(620, 10, 60, 30)
    btn_rhomb = pygame.Rect(690, 10, 80, 30)

    font = pygame.font.SysFont(None, 20)
    start_pos = None

    clock = pygame.time.Clock()
    running = True

    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False

            # НАЖАТИЕ МЫШИ
            if event.type == pygame.MOUSEBUTTONDOWN:
                x, y = event.pos

                # Если нажали на UI
                if y <= 50:
                    if btn_red.collidepoint(x, y): current_color = colors['red']
                    elif btn_green.collidepoint(x, y): current_color = colors['green']
                    elif btn_blue.collidepoint(x, y): current_color = colors['blue']
                    elif btn_black.collidepoint(x, y): current_color = colors['black']

                    elif btn_brush.collidepoint(x, y): mode = 'brush'
                    elif btn_rect.collidepoint(x, y): mode = 'rect'
                    elif btn_circ.collidepoint(x, y): mode = 'circle'
                    elif btn_erase.collidepoint(x, y): mode = 'eraser'

                    # НОВЫЕ режимы
                    elif btn_square.collidepoint(x, y): mode = 'square'
                    elif btn_rtri.collidepoint(x, y): mode = 'rtriangle'
                    elif btn_etri.collidepoint(x, y): mode = 'etriangle'
                    elif btn_rhomb.collidepoint(x, y): mode = 'rhombus'
                else:
                    drawing = True
                    start_pos = event.pos

            # ОТПУСКАНИЕ МЫШИ → рисуем фигуру
            if event.type == pygame.MOUSEBUTTONUP:
                if drawing:
                    drawing = False
                    end_pos = event.pos

                    if start_pos and end_pos:

                        x1, y1 = start_pos
                        x2, y2 = end_pos

                        # --- ПРЯМОУГОЛЬНИК ---
                        if mode == 'rect':
                            r = pygame.Rect(min(x1,x2), min(y1,y2),
                                            abs(x1-x2), abs(y1-y2))
                            pygame.draw.rect(canvas, current_color, r)

                        # --- КРУГ ---
                        elif mode == 'circle':
                            radius = int(((x1-x2)**2 + (y1-y2)**2)**0.5)
                            pygame.draw.circle(canvas, current_color, start_pos, radius)

                        # --- КВАДРАТ ---
                        elif mode == 'square':
                            size = min(abs(x1-x2), abs(y1-y2))
                            r = pygame.Rect(x1, y1, size, size)
                            pygame.draw.rect(canvas, current_color, r)

                        # --- ПРЯМОУГОЛЬНЫЙ ТРЕУГОЛЬНИК ---
                        elif mode == 'rtriangle':
                            points = [(x1, y1), (x2, y2), (x1, y2)]
                            pygame.draw.polygon(canvas, current_color, points)

                        # --- РАВНОСТОРОННИЙ ТРЕУГОЛЬНИК ---
                        elif mode == 'etriangle':
                            side = abs(x2 - x1)
                            height = side * math.sqrt(3) / 2

                            points = [
                                (x1, y1),
                                (x1 + side, y1),
                                (x1 + side/2, y1 - height)
                            ]
                            pygame.draw.polygon(canvas, current_color, points)

                        # --- РОМБ ---
                        elif mode == 'rhombus':
                            cx = (x1 + x2) / 2
                            cy = (y1 + y2) / 2

                            points = [
                                (cx, y1),
                                (x2, cy),
                                (cx, y2),
                                (x1, cy)
                            ]
                            pygame.draw.polygon(canvas, current_color, points)

                start_pos = None

            # ДВИЖЕНИЕ МЫШИ (рисование кистью)
            if event.type == pygame.MOUSEMOTION:
                if drawing:
                    if mode == 'brush':
                        pygame.draw.circle(canvas, current_color, event.pos, 5)
                    elif mode == 'eraser':
                        pygame.draw.circle(canvas, colors['white'], event.pos, 15)

        # Рисуем холст
        screen.blit(canvas, (0, 0))

        # UI
        pygame.draw.rect(screen, (200, 200, 200), ui_rect)

        pygame.draw.rect(screen, colors['red'], btn_red)
        pygame.draw.rect(screen, colors['green'], btn_green)
        pygame.draw.rect(screen, colors['blue'], btn_blue)
        pygame.draw.rect(screen, colors['black'], btn_black)

        # Функция кнопок
        def draw_btn(rect, text, active):
            color = (150,150,150) if active else (220,220,220)
            pygame.draw.rect(screen, color, rect)
            pygame.draw.rect(screen, (0,0,0), rect, 2)
            txt = font.render(text, True, (0,0,0))
            screen.blit(txt, (rect.x+3, rect.y+5))

        # Рисуем кнопки
        draw_btn(btn_brush, "Brush", mode=='brush')
        draw_btn(btn_rect, "Rect", mode=='rect')
        draw_btn(btn_circ, "Circ", mode=='circle')
        draw_btn(btn_erase, "Erase", mode=='eraser')

        draw_btn(btn_square, "Square", mode=='square')
        draw_btn(btn_rtri, "R-Tri", mode=='rtriangle')
        draw_btn(btn_etri, "E-Tri", mode=='etriangle')
        draw_btn(btn_rhomb, "Rhomb", mode=='rhombus')

        pygame.display.flip()
        clock.tick(60)

    pygame.quit()

if __name__ == "__main__":
    main()